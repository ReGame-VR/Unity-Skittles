...
if already_computed == 1
    ...
    if counter == 1 || counter == 2  % positive velocities
        if session == 1 || session == 2
            fileName = sprintf('SolMan Figures/SM_pos_%s_original.mat', hand);
        else
            fileName = sprintf('SolMan Figures/SM_pos_%s_transfer.mat', hand);
        end
    else
        % negative velocities
        if session == 1 || session == 2
            fileName = sprintf('SolMan Figures/SM_neg_%s_original.mat', hand);
        else
            fileName = sprintf('SolMan Figures/SM_neg_%s_transfer.mat', hand);
        end
    end
...
if already_computed == 0
    ...
    if counter == 1 || counter == 2
        if session == 1 || session == 2
            save(sprintf('SolMan Figures/SM_pos_%s_original.mat', hand),'D')
        else
            save(sprintf('SolMan Figures/SM_pos_%s_transfer.mat', hand),'D')
        end
    else
        if session == 1 || session == 2
            save(sprintf('SolMan Figures/SM_neg_%s_original.mat', hand),'D')
        else
            save(sprintf('SolMan Figures/SM_neg_%s_transfer.mat', hand),'D')
        end
    end
